...
def aplicando_translacao(image, tx, ty):
    linha, coluna, _ = image.shape
    matrix_translacao = np.float32([[1, 0, tx], [0, 1, ty]])
    imagem_translacao = cv2.warpAffine(image, matrix_translacao, (coluna, linha))
    return imagem_translacao

def aplicando_rotacao(image, angulo):
    linha, coluna, _ = image.shape
    matrix_rotacao = cv2.getRotationMatrix2D((coluna / 2, linha / 2), angulo, 1)
    imagem_rotacionada = cv2.warpAffine(image, matrix_rotacao, (coluna, linha))
